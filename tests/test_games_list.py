...
def test_default_list_shape(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"limit": 5})
...
def test_pagination_clamping(api, base_url):
    # offset < 0 → 0
    r = api.get(f"{base_url}/api/games", params={"offset": -5, "limit": 1})
    assert r.status_code == 200
    assert r.json()["offset"] == 0

    # limit > 1000 → 1000
    r = api.get(f"{base_url}/api/games", params={"limit": 5000})
    assert r.status_code == 200
...
def test_substring_filter(api, base_url):
    ...
    r = api.get(f"{base_url}/api/games", params={"q": "a", "limit": 5})
    assert r.status_code == 200
...
def test_boolean_filter_is_installed(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"isInstalled": "true", "limit": 5})
    assert r.status_code == 200
    for game in r.json()["items"]:
...
def test_boolean_filter_favorite_false(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"favorite": "false", "limit": 5})
    assert r.status_code == 200
    for game in r.json()["items"]:
...
def test_range_filter_playtime_sorted_desc(api, base_url):
    r = api.get(f"{base_url}/api/games", params={
        "playtimeMin": "0",
        "sort": "-playtime",
        "limit": 5,
    })
...
def test_sort_ascending_by_name_returns_200(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"sort": "name", "limit": 10})
    assert r.status_code == 200
...
def test_sort_asc_and_desc_by_name_differ(api, base_url):
    ...
    """
    import pytest
    asc = api.get(f"{base_url}/api/games", params={"sort": "name", "limit": 100}).json()["items"]
    desc = api.get(f"{base_url}/api/games", params={"sort": "-name", "limit": 100}).json()["items"]
    if len(asc) < 2:
...
def test_unknown_sort_field(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"sort": "nope"})
...
def test_bad_boolean_is_installed(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"isInstalled": "yes"})
    assert_bad_request(
        r,
...
def test_invalid_playtime_range(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"playtimeMin": "1000", "playtimeMax": "500"})
    assert_bad_request(
        r,
...
def test_bad_uuid_in_multi_id(api, base_url):
    ...
    r = api.get(f"{base_url}/api/games", params={"platformIds": f"{valid},not-a-guid"})
    assert_bad_request(
        r,
...
def test_user_score_min_out_of_range_negative(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"userScoreMin": "-5"})
    assert_bad_request(
        r,
        expected_message=(
...
def test_bad_integer_for_limit(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"limit": "abc"})
...
def test_empty_value_for_offset(api, base_url):
    ...
    r = api.get(f"{base_url}/api/games?offset=")
    assert_bad_request(r, expected_message="Empty value for 'offset'.")


def test_bad_iso_date(api, base_url):
    r = api.get(f"{base_url}/api/games", params={"lastActivityAfter": "not-a-date"})
    assert_bad_request(
        r,
